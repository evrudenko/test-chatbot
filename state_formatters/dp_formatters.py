...
def base_response_selector_formatter_service(payload: List):
    ...
    if len(payload) == 3:
        return {"skill_name": payload['skill_name'], "text": payload['text'], "confidence": payload['confidence']}
    elif len(payload) == 5:
        return {
...
def last_utt_annotations(dialog: Dict) -> List[Dict]:
    # Used by: dp_toxic_formatter, sent_segm_formatter, tfidf_formatter, sentiment_classification
    return [{"annotations": [dialog["utterances"][-1]["annotations"]]}]
